...
impl Shaders {
    ...
    fn generate_pipeline_variant(
        ...
        let module = self.module_cache.entry(module_key).or_insert_with(|| {
            ...
            //println!("{src}");
            //println!("----");
            //shaders.preprocessor.reset_defines();
...
#[test]
fn simple() {
    fn s(a: &'static str) -> Cow<'static, str> { Cow::Borrowed(a) }

    println!("{}", generate_shader_source(
        &GeometryDescriptor {
            name: s("tile"),
            source: include_str!("../../shaders/tile.wgsl").into(),
            vertex_attributes: Vec::new(),
            instance_attributes: vec![
                VertexAtribute { name: s("tile_pos"), kind: WgslType::Uint32, },
            ],
            bindings: None,
        },
        Some(&MaskDescriptor {
            name: s("mask::fill"),
            source: include_str!("../../shaders/lib/mask/fill.wgsl").into(),
            varyings: vec![
                Varying { name: s("uv"), kind: WgslType::Float32x2, interpolated: false },
                Varying { name: s("edges_fillrule_backdrop"), kind: WgslType::Uint32x4, interpolated: false },
            ],
            bindings: None,
        }),
        Some(&PatternDescriptor {
            name: s("pattern::gradient"),
            source: include_str!("../../shaders/pattern/simple_gradient.wgsl").into(),
            varyings: vec![
                Varying { name: s("position"), kind: WgslType::Float32x2, interpolated: true },
                Varying { name: s("color0"), kind: WgslType::Float32x4, interpolated: false },
                Varying { name: s("color1"), kind: WgslType::Float32x4, interpolated: false },
                Varying { name: s("dir_offset"), kind: WgslType::Float32x3, interpolated: false },
            ],
            bindings: None,
        }),
        None,
