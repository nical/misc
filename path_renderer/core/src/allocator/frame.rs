...
pub type TlFrameVec<'frame, T> = allocator_api2::vec::Vec<T, &'frame ThreadLocalFrameAllocator>;
...
// TODO: The allocator is quite large. Should contain a pointer to the frame memory.
...
impl ThreadLocalFrameAllocator {
    ...
    pub fn new_vec<T>(&self) -> TlFrameVec<T> {
        TlFrameVec::new_in(self)
    }

    pub fn new_vec_with_capacity<T>(&self, cap: usize) -> TlFrameVec<T> {
        TlFrameVec::with_capacity_in(cap, self)
    }
