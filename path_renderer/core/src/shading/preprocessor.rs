//! A simple WGSL preprocessor
//!
//! This preprocessor adds support for `#import` statements and static #if blocks.

// Since this was initially written, naga got support for constant overrides which
// provides a good chunk of the motivation behind static #if/#else blocks.
// It may be that by now a full preprocessor isn't really necessary or that something
// can be grabbed from crates.io. It's pretty handy though!
...
impl Preprocessor {
    ...
    fn parse_define(&mut self, src: &mut Tokenizer) -> Result<(), SourceError> {
        while src.consume_whitespace() || src.consume_comment() || src.consume_new_line() {}
        ...
        match src.current() {
            ...
            Some(other) => {
                src.parse_error(format!(
                    "Unexpected {:?} in static define name",
                    other.as_str()
                ))?;
                unreachable!();
            }
        ...
        match src.current() {
            ...
            Some(other) => {
                src.parse_error(format!(
                    "Expected '{{', got {:?} after static define name",
                    other.as_str()
                ))?;
            }
            None => {
    ...
    fn parse_static_if_condition(&mut self, src: &mut Tokenizer) -> Result<bool, SourceError> {
        ...
        match src.current() {
            ...
            Some(other) => {
                src.parse_error(format!(
                    "Unexpected {:?} in static if condition",
                    other.as_str()
                ))?;
                unreachable!();
            }
        ...
        match src.current() {
            ...
            Some(other) => {
                src.parse_error(format!(
                    "Expected '{{', got {:?} after static if condition",
                    other.as_str()
                ))?;
            }
            None => {
...
impl<'l> Token<'l> {
    fn as_str(&self) -> &'l str {
        match self {
            Token::Comment(text) | Token::Basic(text) | Token::WhiteSpace(text) => text,
            Token::NewLine => &"\n",
            Token::OpenBracket => &"{",
            Token::CloseBracket => &"}",
            Token::Import => &"#import",
...
fn nested_static_if() {
    let mut preprocessor = Preprocessor::new();
    let src =
        &"[#if FOO { #if BAR {foo_bar} } #else { #if BAZ {not_foo_baz} #else {not_foo_not_baz} }]";
...
#import A
    ...
    let output = loader.preprocess("A", "#import A", &lib).unwrap();
