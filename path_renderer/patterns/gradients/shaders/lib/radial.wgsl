...
// Note: the last field (w) of the radial gradient data is unused.
...
fn read_css_radial_gradient(position: vec2f, address: u32) -> vec4f {
    ...
    return vec4f(normalized_pos, start_radius, 0.0);
}

fn compute_css_radial_gradient_offset(position_and_start: vec4f) -> f32 {
