...
use crate::graph::{Allocation, FrameGraph, GraphBindings, GraphSystem, PassRenderContext};
...
impl Instance {
    ...
    pub fn render_frame(
        ...
        for (idx, renderer) in renderers.iter_mut().enumerate() {
            renderer.prepare(&mut ctx, &frame.render_nodes.passes());
            let end = Instant::now();
        ...
        for pass in &passes {
            match pass.system {
                0 => {
                    frame.render_nodes.render(&mut ctx, renderers, *pass);
                }
                _ => {
                    unimplemented!()
                }
